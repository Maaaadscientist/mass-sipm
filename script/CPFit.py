...
formula2 = f"exp(-mu) * exp(-pow(sigQ - ped, 2)/(2 * pow(sigma0, 2))) / sigma0  + \
                     exp(-mu) * (1 - lambda) * exp(-pow(sigQ - (ped + 1 * gain), 2)/(2 * pow(sigma1, 2))) / sigma1  + \
                     ({printCP(2)}) / TMath::Factorial(2) * exp(-mu ) * exp(-pow(sigQ - (ped + 2 * gain), 2)/(2 * pow(sigma2, 2))) /  sigma2   + \
                     ({printCP(3)}) / TMath::Factorial(3) * exp(-mu ) * exp(-pow(sigQ - (ped + 3 * gain), 2)/(2 * pow(sigma3, 2))) /  sigma3  + \
                     ({printCP(4)}) / TMath::Factorial(4) * exp(-mu ) * exp(-pow(sigQ - (ped + 4 * gain), 2)/(2 * pow(sigma4, 2))) / sigma4  + \
                     ({printCP(5)}) / TMath::Factorial(5) * exp(-mu ) * exp(-pow(sigQ - (ped + 5 * gain), 2)/(2 * pow(sigma5, 2))) / sigma5  + \
                     ({printCP(6)}) / TMath::Factorial(6) * exp(-mu ) * exp(-pow(sigQ - (ped + 6 * gain), 2)/(2 * pow(sigma6, 2))) / sigma6  + \
                     ({printCP(7)}) / TMath::Factorial(7) * exp(-mu ) * exp(-pow(sigQ - (ped + 7 * gain), 2)/(2 * pow(sigma7, 2))) / sigma7  + \
                     ({printCP(8)}) / TMath::Factorial(8) * exp(-mu ) * exp(-pow(sigQ - (ped + 8 * gain), 2)/(2 * pow(sigma8, 2))) / sigma8 "
...
param_box.AddText(f"p = {lambda_.getVal():.3f} #pm {lambda_.getError():.3f}")
