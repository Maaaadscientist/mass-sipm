...
for ch in range(1, 17):
    for ov in range(1, 7):
        ...
        script += 'charge_fit_command="$python fit.py ${input_file} signal sigQ_ch${po} ${output_file}"\n'
        script += 'echo "Executing command: ${charge_fit_command}"\n'
        script += '$charge_fit_command\n'
        script += '\n'
        script += 'cd -\n'
        with open(f'{output_dir}/jobs/charge_fit_tile_ch{ch}_po{po}_ov{ov}.sh','w') as file_tmp:
