...
if __name__ == '__main__':
    ...
    for file in root_files:
        ...
        plt.clf()
        # Appending elements to all columns
        deviation_x.append(dx[min_pos_good])
        deviation_y.append(dy[min_pos_good])
        deviation_linked_x.append(dx[min_pos])
        deviation_linked_y.append(dy[min_pos])
        best_pos_light_intensity.append(rbf(dx[min_pos], dy[min_pos]))
        valid_run_list.append(run_number)

        with open("completed.txt","a") as logfile:
            logfile.write(str(int(run_number))+"\n")
    # Create a DataFrame
    data = {'dev_x': deviation_x, 'dev_y': deviation_y, 'dev_linked_x': deviation_linked_x, 'dev_linked_y':deviation_linked_y, 'light_central':best_pos_light_intensity, 'run_number':valid_run_list}
    df = pd.DataFrame(data)
    
    # Write to CSV file
    df.to_csv('output.csv', index=True)
    print("invalid runs:")
    for i in range(len(invalid_run_list)):
        print(invalid_run_list[i])
