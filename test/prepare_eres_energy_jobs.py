...
job_split = 50
N_events = 1000
root_dir = "/junofs/users/wanghanwen/e_resolution_energy"
...
for ov in ov_list:
    if ov == -1:
        ...
        ov_str = f"{ov:0.1f}"
    for e in [1.0 + 0.5* i for i in range(11)]:
        for njob in range(job_split):
            job_content = f"$PYTHON /workfs2/juno/wanghanwen/sipm-massive/test/e_res.py --N {N_events} --energy {e} --ov {ov:.1f} --input /junofs/users/wanghanwen/bychannel.csv --output {output_dir}/hist_e_{e}_{ov_str}_{njob}.root --seed {njob} --level {level}"
            with open(f"{job_dir}/hist_e_{e}_{ov_str}_{njob}.sh", "w") as file1:
                file1.write(script)
