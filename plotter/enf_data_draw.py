...
lower_edges = {"3v":1.1, "3p5v":1.1, "4v":1.1, "4p5v":1.1, "vop":1.1}
upper_edges = {"3v":1.4, "3p5v":1.4, "4v":1.4, "4p5v":1.4, "vop":1.4}
step = 0.02
enf_data_max = {v: np.zeros(n, dtype=float) for v in voltages}
enf_data_max_err = {v: np.zeros(n, dtype=float) for v in voltages}
enf_data_min = {v: np.zeros(n, dtype=float) for v in voltages}
enf_data_min_err = {v: np.zeros(n, dtype=float) for v in voltages}
enf_data_mean = {v: np.zeros(n, dtype=float) for v in voltages}
enf_data_mean_err = {v: np.zeros(n, dtype=float) for v in voltages}
...
for i, event in enumerate(tree):
    ...
    tsn[i] = event.tsn
    for v in voltages:
        enf_data_max[v][i] = abs(getattr(event, f"enf_data_{v}_max"))
        enf_data_max_err[v][i] = abs(getattr(event, f"enf_data_{v}_max_err"))
        enf_data_min[v][i] = abs(getattr(event, f"enf_data_{v}_min"))
        enf_data_min_err[v][i] = abs(getattr(event, f"enf_data_{v}_min_err"))
        enf_data_mean[v][i] = abs(getattr(event, f"enf_data_{v}_mean"))
        enf_data_mean_err[v][i] = abs(getattr(event, f"enf_data_{v}_mean_err"))
        #setattr(enf_data_max[v], i, abs(getattr(event, f"enf_data_{v}_max")))
        #setattr(enf_data_max_err[v], i, abs(getattr(event, f"enf_data_{v}_max_err")))
        #setattr(enf_data_min[v], i, abs(getattr(event, f"enf_data_{v}_min")))
        #setattr(enf_data_min_err[v], i, abs(getattr(event, f"enf_data_{v}_min_err")))
        #setattr(enf_data_mean[v], i, abs(getattr(event, f"enf_data_{v}_mean")))
        #setattr(enf_data_mean_err[v], i, abs(getattr(event, f"enf_data_{v}_mean_err")))
...
for idx,v in enumerate(voltages):
    ...
    plt.errorbar(count, enf_data_max[v], yerr=enf_data_max_err[v], fmt='^', label="Max", capsize=3, color=maxcolor, markersize=3, alpha=0.7, mfc='none', elinewidth=0.5, markeredgewidth=0.5)
    
    # Plot min values
    plt.errorbar(count, enf_data_min[v], yerr=enf_data_min_err[v], fmt='v', label="Min", capsize=3, color=mincolor, markersize=3, alpha=0.7, elinewidth=0.5, mfc='none', mec=meancolor, markeredgewidth=0.5)
    
    # Plot mean values
    plt.errorbar(count, enf_data_mean[v], yerr=enf_data_mean_err[v], fmt='o', label="Mean", capsize=3, color=meancolor,markersize=3, alpha=0.7, elinewidth=0.5, mfc='none', mec=meancolor, markeredgewidth=0.5)
    ...
    if idx == 0:
        plt.title(f"Electronic Noise Factor of different SiPM tiles", fontsize=titlesize)
    ...
    plt.ylabel("ENF", fontsize=labelsize)
...
plt.savefig("enf_data_tiles.pdf")
...
for v in voltages:
    ...
    hist_max, _ = np.histogram(enf_data_max[v], bins=bins)
    hist_min, _ = np.histogram(enf_data_min[v], bins=bins)
    hist_mean, _ = np.histogram(enf_data_mean[v], bins=bins)
    ...
    ov = v.replace("v","V").replace("p", ".") if v!="vop" else "$\\mathrm{V}_{op}$"
    plt.xlabel("ENF",fontsize=titlesize)
    plt.ylabel("Number of Tiles",fontsize=titlesize)
    plt.legend(fontsize=titlesize)
    plt.grid(True, which="both", linestyle="--", linewidth=0.5)
    plt.title(f"Electronic Noise Factor ({ov})", fontsize=titlesize+6)
    plt.tight_layout()
    plt.subplots_adjust(left=0.08)
    
    plt.savefig(f"enf_data_distribution_{v}.pdf")
    plt.clf()
