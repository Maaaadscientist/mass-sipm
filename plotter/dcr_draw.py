...
upper_edges = {"3v":120, "3p5v":150, "4v":180, "4p5v":200, "vop":150}
step = 10
dcr_max = {v: np.zeros(n, dtype=float) for v in voltages}
dcr_max_err = {v: np.zeros(n, dtype=float) for v in voltages}
dcr_min = {v: np.zeros(n, dtype=float) for v in voltages}
dcr_min_err = {v: np.zeros(n, dtype=float) for v in voltages}
dcr_mean = {v: np.zeros(n, dtype=float) for v in voltages}
dcr_mean_err = {v: np.zeros(n, dtype=float) for v in voltages}
...
for i, event in enumerate(tree):
    ...
    tsn[i] = event.tsn
    for v in voltages:
        dcr_max[v][i] = abs(getattr(event, f"dcr_{v}_max"))
        dcr_max_err[v][i] = abs(getattr(event, f"dcr_{v}_max_err"))
        dcr_min[v][i] = abs(getattr(event, f"dcr_{v}_min"))
        dcr_min_err[v][i] = abs(getattr(event, f"dcr_{v}_min_err"))
        dcr_mean[v][i] = abs(getattr(event, f"dcr_{v}_mean"))
        dcr_mean_err[v][i] = abs(getattr(event, f"dcr_{v}_mean_err"))
        #setattr(dcr_max[v], i, abs(getattr(event, f"dcr_{v}_max")))
        #setattr(dcr_max_err[v], i, abs(getattr(event, f"dcr_{v}_max_err")))
        #setattr(dcr_min[v], i, abs(getattr(event, f"dcr_{v}_min")))
        #setattr(dcr_min_err[v], i, abs(getattr(event, f"dcr_{v}_min_err")))
        #setattr(dcr_mean[v], i, abs(getattr(event, f"dcr_{v}_mean")))
        #setattr(dcr_mean_err[v], i, abs(getattr(event, f"dcr_{v}_mean_err")))
...
for idx,v in enumerate(voltages):
    ...
    plt.errorbar(count, dcr_max[v], yerr=dcr_max_err[v], fmt='^', label="Max", capsize=3, color=maxcolor, markersize=3, alpha=0.7, mfc='none', elinewidth=0.5, markeredgewidth=0.5)
    
    # Plot min values
    plt.errorbar(count, dcr_min[v], yerr=dcr_min_err[v], fmt='v', label="Min", capsize=3, color=mincolor, markersize=3, alpha=0.7, elinewidth=0.5, mfc='none', mec=meancolor, markeredgewidth=0.5)
    
    # Plot mean values
    plt.errorbar(count, dcr_mean[v], yerr=dcr_mean_err[v], fmt='o', label="Mean", capsize=3, color=meancolor,markersize=3, alpha=0.7, elinewidth=0.5, mfc='none', mec=meancolor, markeredgewidth=0.5)
    
    plt.axhline(y=41.7, color='lime', linestyle='--', linewidth=2.5, label="Req. $\\leq$ 41.7", zorder=5)
    ...
    if idx == 0:
        plt.title(f"Dark count rate of different SiPM tiles", fontsize=titlesize)
    ...
    plt.ylabel("DCR (Hz/$\mathrm{mm}^2$)", fontsize=labelsize)
...
for v in voltages:
    ...
    hist_max, _ = np.histogram(dcr_max[v], bins=bins)
    hist_min, _ = np.histogram(dcr_min[v], bins=bins)
    hist_mean, _ = np.histogram(dcr_mean[v], bins=bins)
    ...
    plt.axvline(x=41.7, color='lime', linestyle='--', linewidth=5, label="Req. $\\leq$ 41.7", zorder=5)
    ...
    ov = v.replace("v","V").replace("p", ".") if v!="vop" else "$\\mathrm{V}_{op}$"
    plt.xlabel("DCR (Hz/$\mathrm{mm}^2$)",fontsize=titlesize)
    plt.ylabel("Number of Tiles",fontsize=titlesize)
    plt.legend(fontsize=titlesize)
    plt.grid(True, which="both", linestyle="--", linewidth=0.5)
    plt.title(f"Dark count rate ({ov})", fontsize=titlesize+6)
    plt.tight_layout()
    plt.subplots_adjust(left=0.08)
    
    plt.savefig(f"dcr_distribution_{v}.pdf")
    plt.clf()
