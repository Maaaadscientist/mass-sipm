...
def peak_pos(k, ped, gain):
    ...
    return f"({ped} + {k} * {gain})"
def peak_pos_ap(k, i, ped, gain, ap_gain):
    if k < 0 or type(k) is not int:
        raise ValueError("k should be a non-negative integer!")
    return f"({ped} + {k} * {gain} + {i} * {ap_gain})"
...
for k in range(1, 15):
    ...
    Gaussk_0 = gauss("sigQ", x_k_0, sigma_n_0)
    bino_0 = binominalI(k, 0, "alpha")
    ...
    if k >= 1:
        for i in range(1, k+1):
            ...
            Gaussk_i = gauss("sigQ", x_k_i, sigma_n_i)
            #normal_conv = expo_convolution("sigQ", x_k, "beta", i)
            formula2 += f" + {bino_i} * {Gaussk_i}"
            print(k,i,f" + {bino_i} * {Gaussk_i}")
    formula2 += "))"
        
    #formula2 += f" + {GPk} * {Gaussk}"
with open("fit_studies/compound_pdf.py","w") as file0:
