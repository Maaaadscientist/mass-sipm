...
for file_path in files:
    ...
    if run_type == "main":
        ...
        script_tmp += f'/junofs/users/wanghanwen/sipm-massive/bin/skim-signal -i {file_name} -c test.yaml -r {run_number} -v {ov} -t {run_type}_{sipm_type}_ch{channel} -o {output_dir}/{output_name}.root\n'
        script_tmp += 'sleep 5\n'
        ...
        script_tmp += '. ./env_lcg.sh\n'
        output_name = "_".join(components[0:-3])
        script_tmp += f'/junofs/users/wanghanwen/sipm-massive/bin/skim-signal -i {file_name} -c test.yaml -r {run_number} -v 0 -t {run_type}_{sipm_type}_ch{channel} -o {output_dir}/{output_name}.root\n'
        script_tmp += 'sleep 5\n'
        script_tmp += f'rm -f {file_name}\n'
        script_tmp += 'cd -\n'
        with open(f'{output_dir}/jobs/{output_name}.sh','w') as file_tmp:
